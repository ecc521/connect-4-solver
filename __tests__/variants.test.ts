...
describe("Extended Game Variants (Connect 5 & Wraparound)", () => {
  it("evaluates Connect-5 winning positions correctly", async () => {
    const solver = new NodeConnect4Solver({ width: 8, height: 8, align: 5, wrap: false, heuristic: false });
    await solver.init();
  ...
  it("evaluates Wraparound winning positions correctly", async () => {
    ...
    const solver = new NodeConnect4Solver({ width: 7, height: 6, align: 4, wrap: true, heuristic: false });
    await solver.init();
  ...
  it("evaluates Connect-5 Wraparound combined correctly", async () => {
    ...
    const solver = new NodeConnect4Solver({ width: 8, height: 8, align: 5, wrap: true, heuristic: false });
    await solver.init();
