mkdir -p wasm-out
EXPORTS='["_free", "_malloc", "_createCache", "_destroyCache", "_createSolver", "_destroySolver", "_stopSolver", "_createBookFromBuffer", "_destroyBook", "_getBookScore", "_solveExact", "_analyzeExact", "_getNodeCount"]'
...
emcc -O3 -std=c++20 -msimd128 -DNDEBUG -fwasm-exceptions -DUSE_PTHREADS=1 -pthread -DCACHE_BUCKET_SIZE=2 -o wasm-out/analyze_threaded.js native/analyze.cpp -s NO_EXIT_RUNTIME=1 -s EXPORTED_FUNCTIONS="$EXPORTS" -s EXPORTED_RUNTIME_METHODS='["FS", "UTF8ToString", "stringToNewUTF8", "getValue", "wasmMemory"]' -s INITIAL_MEMORY=32MB -s MAXIMUM_MEMORY=4GB -s ALLOW_MEMORY_GROWTH -s WASM=1 -s MODULARIZE=1 -s EXPORT_ES6=1 -s EXPORT_NAME="createThreadedModule" -s PTHREAD_POOL_SIZE=4 -s PTHREAD_POOL_SIZE_STRICT=0 -s STACK_SIZE=1048576
