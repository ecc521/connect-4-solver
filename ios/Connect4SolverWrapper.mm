...
+ (NSString*)createCache:(int)w height:(int)h sizeBytes:(double)sizeBytes align:(int)align wrap:(BOOL)wrap {
...
+ (NSString*)createSolver:(int)w height:(int)h cachePtrStr:(NSString*)cachePtrStr align:(int)align wrap:(BOOL)wrap {
    auto cache = stringToPtr<GameSolver::Connect4::Cache>(cachePtrStr);
    void* ptr = dispatch<void*>(w, h, align, wrap, [&](auto tag) {
        using Size = typename decltype(tag)::type;
...
+ (void)destroySolver:(NSString*)solverPtrStr width:(int)w height:(int)h align:(int)align wrap:(BOOL)wrap {
...
+ (void)stop:(NSString*)solverPtrStr width:(int)w height:(int)h align:(int)align wrap:(BOOL)wrap {
    void* solver = stringToPtr<void>(solverPtrStr);
    if (!solver) return;
    dispatch_void(w, h, align, wrap, [&](auto tag) {
        using Size = typename decltype(tag)::type;
