...
int32_t* runAnalysis(CoreSolver& solver, const char* positionCharArr, bool weak, int threads, void* book_ptr, double timeout_ms) {
...
template <typename CoreSolver, typename CorePosition, int W, int H, typename CoreBook>
int32_t* runSolve(CoreSolver& solver, const char* positionCharArr, bool weak, void* book_ptr, double timeout_ms) {
  std::string positionString(positionCharArr);
  CorePosition P;
...
int32_t* runHeuristicAnalysis(CoreSolver& solver, const char* positionCharArr, int max_depth, int threads, double timeout_ms) {
  ...
  if(P.play(positionString) != positionString.size()) {
    ...
    if (timeout_ms > 0) {
        end_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count() + timeout_ms;
    }
...
EMSCRIPTEN_KEEPALIVE void* createCache(int w, int h, size_t bytes, bool is_heuristic) {
    if (is_heuristic) {
        ...
        if (w == 6 && h == 8) return GameSolver::Connect4::HeuristicSolver<6, 8>::createCache(bytes).release();
        ...
        if (w == 7 && h == 8) return GameSolver::Connect4::HeuristicSolver<7, 8>::createCache(bytes).release();
        ...
        if (w == 9 && h == 8) return GameSolver::Connect4::HeuristicSolver<9, 8>::createCache(bytes).release();
        ...
        if (w == 10 && h == 8) return GameSolver::Connect4::HeuristicSolver<10, 8>::createCache(bytes).release();
        ...
        if (w == 11 && h == 8) return GameSolver::Connect4::HeuristicSolver<11, 8>::createCache(bytes).release();
        ...
        if (w == 12 && h == 8) return GameSolver::Connect4::HeuristicSolver<12, 8>::createCache(bytes).release();
...
EMSCRIPTEN_KEEPALIVE void* createSolver(int w, int h, void* cache, bool is_heuristic) {
    ...
    if (is_heuristic) {
        ...
        if (w == 6 && h == 8) return GameSolver::Connect4::HeuristicSolver<6, 8>::createWithCache(c).release();
        ...
        if (w == 7 && h == 8) return GameSolver::Connect4::HeuristicSolver<7, 8>::createWithCache(c).release();
        ...
        if (w == 9 && h == 8) return GameSolver::Connect4::HeuristicSolver<9, 8>::createWithCache(c).release();
        ...
        if (w == 10 && h == 8) return GameSolver::Connect4::HeuristicSolver<10, 8>::createWithCache(c).release();
        ...
        if (w == 11 && h == 8) return GameSolver::Connect4::HeuristicSolver<11, 8>::createWithCache(c).release();
        ...
        if (w == 12 && h == 8) return GameSolver::Connect4::HeuristicSolver<12, 8>::createWithCache(c).release();
...
EMSCRIPTEN_KEEPALIVE void stopSolver(int w, int h, void* solver, bool is_heuristic) {
    if (is_heuristic) {
...
EMSCRIPTEN_KEEPALIVE void* createBook(int w, int h, const char* path) {
    ...
    void* ptr = nullptr;
    try {
        #define CREATE_FILE_ACTION(NS, W, H, _) ptr = NS::GameSolver::Connect4::OpeningBookBase<W, H>::load(p, W, H).release();
        DISPATCH_BOOK(w, h, nullptr, CREATE_FILE_ACTION);
...
EMSCRIPTEN_KEEPALIVE void* createBookFromBuffer(int w, int h, const uint8_t* data, size_t size) {
    void* ptr = nullptr;
    try {
        #define CREATE_BUF_ACTION(NS, W, H, _) ptr = NS::GameSolver::Connect4::OpeningBookBase<W, H>::load_from_memory(data, size, W, H).release();
...
EMSCRIPTEN_KEEPALIVE void* convertBookToDense(int w, int h, void* ptr) {
...
EMSCRIPTEN_KEEPALIVE void* convertBookToEF(int w, int h, void* ptr) {
    void* new_ptr = nullptr;
    try {
...
EMSCRIPTEN_KEEPALIVE const char* getBookFormat(int w, int h, void* ptr) {
    ...
    // Allocate a heap string to return to JS
    char* ret = (char*)malloc(strlen(format) + 1);
    strcpy(ret, format);
    return ret;
}

EMSCRIPTEN_KEEPALIVE int getBookScore(int w, int h, void* ptr, const char* pos_str) {
    int score = -32000;
    std::string pos(pos_str);
    try {
        #define GET_SCORE_ACTION(NS, W, H, b) \
            { \
                NS::GameSolver::Connect4::Position P; \
                if (P.play(pos) == pos.length()) { \
                    int val = b->get(P); \
...
EMSCRIPTEN_KEEPALIVE int32_t* analyzeExact(int w, int h, void* solver, const char* pos, bool weak, int threads, void* book_ptr, double timeout_ms) {
    if (w == 6 && h == 5) return runAnalysis<C4_6x5::GameSolver::Connect4::Solver, C4_6x5::GameSolver::Connect4::Position, 6, 5, C4_6x5::GameSolver::Connect4::OpeningBookBase<6, 5>>(*static_cast<C4_6x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 6) return runAnalysis<C4_6x6::GameSolver::Connect4::Solver, C4_6x6::GameSolver::Connect4::Position, 6, 6, C4_6x6::GameSolver::Connect4::OpeningBookBase<6, 6>>(*static_cast<C4_6x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 6) return runAnalysis<C4_7x6::GameSolver::Connect4::Solver, C4_7x6::GameSolver::Connect4::Position, 7, 6, C4_7x6::GameSolver::Connect4::OpeningBookBase<7, 6>>(*static_cast<C4_7x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 7) return runAnalysis<C4_7x7::GameSolver::Connect4::Solver, C4_7x7::GameSolver::Connect4::Position, 7, 7, C4_7x7::GameSolver::Connect4::OpeningBookBase<7, 7>>(*static_cast<C4_7x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 6) return runAnalysis<C4_8x6::GameSolver::Connect4::Solver, C4_8x6::GameSolver::Connect4::Position, C4_8x6::GameSolver::Connect4::Position::WIDTH, C4_8x6::GameSolver::Connect4::Position::HEIGHT, C4_8x6::GameSolver::Connect4::OpeningBookBase<8, 6>>(*static_cast<C4_8x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 4) return runAnalysis<C4_4x4::GameSolver::Connect4::Solver, C4_4x4::GameSolver::Connect4::Position, C4_4x4::GameSolver::Connect4::Position::WIDTH, C4_4x4::GameSolver::Connect4::Position::HEIGHT, C4_4x4::GameSolver::Connect4::OpeningBookBase<4, 4>>(*static_cast<C4_4x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 5) return runAnalysis<C4_4x5::GameSolver::Connect4::Solver, C4_4x5::GameSolver::Connect4::Position, C4_4x5::GameSolver::Connect4::Position::WIDTH, C4_4x5::GameSolver::Connect4::Position::HEIGHT, C4_4x5::GameSolver::Connect4::OpeningBookBase<4, 5>>(*static_cast<C4_4x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 6) return runAnalysis<C4_4x6::GameSolver::Connect4::Solver, C4_4x6::GameSolver::Connect4::Position, C4_4x6::GameSolver::Connect4::Position::WIDTH, C4_4x6::GameSolver::Connect4::Position::HEIGHT, C4_4x6::GameSolver::Connect4::OpeningBookBase<4, 6>>(*static_cast<C4_4x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 7) return runAnalysis<C4_4x7::GameSolver::Connect4::Solver, C4_4x7::GameSolver::Connect4::Position, C4_4x7::GameSolver::Connect4::Position::WIDTH, C4_4x7::GameSolver::Connect4::Position::HEIGHT, C4_4x7::GameSolver::Connect4::OpeningBookBase<4, 7>>(*static_cast<C4_4x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 8) return runAnalysis<C4_4x8::GameSolver::Connect4::Solver, C4_4x8::GameSolver::Connect4::Position, C4_4x8::GameSolver::Connect4::Position::WIDTH, C4_4x8::GameSolver::Connect4::Position::HEIGHT, C4_4x8::GameSolver::Connect4::OpeningBookBase<4, 8>>(*static_cast<C4_4x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 9) return runAnalysis<C4_4x9::GameSolver::Connect4::Solver, C4_4x9::GameSolver::Connect4::Position, C4_4x9::GameSolver::Connect4::Position::WIDTH, C4_4x9::GameSolver::Connect4::Position::HEIGHT, C4_4x9::GameSolver::Connect4::OpeningBookBase<4, 9>>(*static_cast<C4_4x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 10) return runAnalysis<C4_4x10::GameSolver::Connect4::Solver, C4_4x10::GameSolver::Connect4::Position, C4_4x10::GameSolver::Connect4::Position::WIDTH, C4_4x10::GameSolver::Connect4::Position::HEIGHT, C4_4x10::GameSolver::Connect4::OpeningBookBase<4, 10>>(*static_cast<C4_4x10::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 11) return runAnalysis<C4_4x11::GameSolver::Connect4::Solver, C4_4x11::GameSolver::Connect4::Position, C4_4x11::GameSolver::Connect4::Position::WIDTH, C4_4x11::GameSolver::Connect4::Position::HEIGHT, C4_4x11::GameSolver::Connect4::OpeningBookBase<4, 11>>(*static_cast<C4_4x11::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 4 && h == 12) return runAnalysis<C4_4x12::GameSolver::Connect4::Solver, C4_4x12::GameSolver::Connect4::Position, C4_4x12::GameSolver::Connect4::Position::WIDTH, C4_4x12::GameSolver::Connect4::Position::HEIGHT, C4_4x12::GameSolver::Connect4::OpeningBookBase<4, 12>>(*static_cast<C4_4x12::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 4) return runAnalysis<C4_5x4::GameSolver::Connect4::Solver, C4_5x4::GameSolver::Connect4::Position, C4_5x4::GameSolver::Connect4::Position::WIDTH, C4_5x4::GameSolver::Connect4::Position::HEIGHT, C4_5x4::GameSolver::Connect4::OpeningBookBase<5, 4>>(*static_cast<C4_5x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 5) return runAnalysis<C4_5x5::GameSolver::Connect4::Solver, C4_5x5::GameSolver::Connect4::Position, C4_5x5::GameSolver::Connect4::Position::WIDTH, C4_5x5::GameSolver::Connect4::Position::HEIGHT, C4_5x5::GameSolver::Connect4::OpeningBookBase<5, 5>>(*static_cast<C4_5x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 6) return runAnalysis<C4_5x6::GameSolver::Connect4::Solver, C4_5x6::GameSolver::Connect4::Position, C4_5x6::GameSolver::Connect4::Position::WIDTH, C4_5x6::GameSolver::Connect4::Position::HEIGHT, C4_5x6::GameSolver::Connect4::OpeningBookBase<5, 6>>(*static_cast<C4_5x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 7) return runAnalysis<C4_5x7::GameSolver::Connect4::Solver, C4_5x7::GameSolver::Connect4::Position, C4_5x7::GameSolver::Connect4::Position::WIDTH, C4_5x7::GameSolver::Connect4::Position::HEIGHT, C4_5x7::GameSolver::Connect4::OpeningBookBase<5, 7>>(*static_cast<C4_5x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 8) return runAnalysis<C4_5x8::GameSolver::Connect4::Solver, C4_5x8::GameSolver::Connect4::Position, C4_5x8::GameSolver::Connect4::Position::WIDTH, C4_5x8::GameSolver::Connect4::Position::HEIGHT, C4_5x8::GameSolver::Connect4::OpeningBookBase<5, 8>>(*static_cast<C4_5x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 9) return runAnalysis<C4_5x9::GameSolver::Connect4::Solver, C4_5x9::GameSolver::Connect4::Position, C4_5x9::GameSolver::Connect4::Position::WIDTH, C4_5x9::GameSolver::Connect4::Position::HEIGHT, C4_5x9::GameSolver::Connect4::OpeningBookBase<5, 9>>(*static_cast<C4_5x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 10) return runAnalysis<C4_5x10::GameSolver::Connect4::Solver, C4_5x10::GameSolver::Connect4::Position, C4_5x10::GameSolver::Connect4::Position::WIDTH, C4_5x10::GameSolver::Connect4::Position::HEIGHT, C4_5x10::GameSolver::Connect4::OpeningBookBase<5, 10>>(*static_cast<C4_5x10::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 11) return runAnalysis<C4_5x11::GameSolver::Connect4::Solver, C4_5x11::GameSolver::Connect4::Position, C4_5x11::GameSolver::Connect4::Position::WIDTH, C4_5x11::GameSolver::Connect4::Position::HEIGHT, C4_5x11::GameSolver::Connect4::OpeningBookBase<5, 11>>(*static_cast<C4_5x11::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 5 && h == 12) return runAnalysis<C4_5x12::GameSolver::Connect4::Solver, C4_5x12::GameSolver::Connect4::Position, C4_5x12::GameSolver::Connect4::Position::WIDTH, C4_5x12::GameSolver::Connect4::Position::HEIGHT, C4_5x12::GameSolver::Connect4::OpeningBookBase<5, 12>>(*static_cast<C4_5x12::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 4) return runAnalysis<C4_6x4::GameSolver::Connect4::Solver, C4_6x4::GameSolver::Connect4::Position, C4_6x4::GameSolver::Connect4::Position::WIDTH, C4_6x4::GameSolver::Connect4::Position::HEIGHT, C4_6x4::GameSolver::Connect4::OpeningBookBase<6, 4>>(*static_cast<C4_6x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 7) return runAnalysis<C4_6x7::GameSolver::Connect4::Solver, C4_6x7::GameSolver::Connect4::Position, C4_6x7::GameSolver::Connect4::Position::WIDTH, C4_6x7::GameSolver::Connect4::Position::HEIGHT, C4_6x7::GameSolver::Connect4::OpeningBookBase<6, 7>>(*static_cast<C4_6x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 8) return runAnalysis<C4_6x8::GameSolver::Connect4::Solver, C4_6x8::GameSolver::Connect4::Position, C4_6x8::GameSolver::Connect4::Position::WIDTH, C4_6x8::GameSolver::Connect4::Position::HEIGHT, C4_6x8::GameSolver::Connect4::OpeningBookBase<6, 8>>(*static_cast<C4_6x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 9) return runAnalysis<C4_6x9::GameSolver::Connect4::Solver, C4_6x9::GameSolver::Connect4::Position, C4_6x9::GameSolver::Connect4::Position::WIDTH, C4_6x9::GameSolver::Connect4::Position::HEIGHT, C4_6x9::GameSolver::Connect4::OpeningBookBase<6, 9>>(*static_cast<C4_6x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 10) return runAnalysis<C4_6x10::GameSolver::Connect4::Solver, C4_6x10::GameSolver::Connect4::Position, C4_6x10::GameSolver::Connect4::Position::WIDTH, C4_6x10::GameSolver::Connect4::Position::HEIGHT, C4_6x10::GameSolver::Connect4::OpeningBookBase<6, 10>>(*static_cast<C4_6x10::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 11) return runAnalysis<C4_6x11::GameSolver::Connect4::Solver, C4_6x11::GameSolver::Connect4::Position, C4_6x11::GameSolver::Connect4::Position::WIDTH, C4_6x11::GameSolver::Connect4::Position::HEIGHT, C4_6x11::GameSolver::Connect4::OpeningBookBase<6, 11>>(*static_cast<C4_6x11::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 6 && h == 12) return runAnalysis<C4_6x12::GameSolver::Connect4::Solver, C4_6x12::GameSolver::Connect4::Position, C4_6x12::GameSolver::Connect4::Position::WIDTH, C4_6x12::GameSolver::Connect4::Position::HEIGHT, C4_6x12::GameSolver::Connect4::OpeningBookBase<6, 12>>(*static_cast<C4_6x12::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 4) return runAnalysis<C4_7x4::GameSolver::Connect4::Solver, C4_7x4::GameSolver::Connect4::Position, C4_7x4::GameSolver::Connect4::Position::WIDTH, C4_7x4::GameSolver::Connect4::Position::HEIGHT, C4_7x4::GameSolver::Connect4::OpeningBookBase<7, 4>>(*static_cast<C4_7x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 5) return runAnalysis<C4_7x5::GameSolver::Connect4::Solver, C4_7x5::GameSolver::Connect4::Position, C4_7x5::GameSolver::Connect4::Position::WIDTH, C4_7x5::GameSolver::Connect4::Position::HEIGHT, C4_7x5::GameSolver::Connect4::OpeningBookBase<7, 5>>(*static_cast<C4_7x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 8) return runAnalysis<C4_7x8::GameSolver::Connect4::Solver, C4_7x8::GameSolver::Connect4::Position, C4_7x8::GameSolver::Connect4::Position::WIDTH, C4_7x8::GameSolver::Connect4::Position::HEIGHT, C4_7x8::GameSolver::Connect4::OpeningBookBase<7, 8>>(*static_cast<C4_7x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 9) return runAnalysis<C4_7x9::GameSolver::Connect4::Solver, C4_7x9::GameSolver::Connect4::Position, C4_7x9::GameSolver::Connect4::Position::WIDTH, C4_7x9::GameSolver::Connect4::Position::HEIGHT, C4_7x9::GameSolver::Connect4::OpeningBookBase<7, 9>>(*static_cast<C4_7x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 10) return runAnalysis<C4_7x10::GameSolver::Connect4::Solver, C4_7x10::GameSolver::Connect4::Position, C4_7x10::GameSolver::Connect4::Position::WIDTH, C4_7x10::GameSolver::Connect4::Position::HEIGHT, C4_7x10::GameSolver::Connect4::OpeningBookBase<7, 10>>(*static_cast<C4_7x10::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 11) return runAnalysis<C4_7x11::GameSolver::Connect4::Solver, C4_7x11::GameSolver::Connect4::Position, C4_7x11::GameSolver::Connect4::Position::WIDTH, C4_7x11::GameSolver::Connect4::Position::HEIGHT, C4_7x11::GameSolver::Connect4::OpeningBookBase<7, 11>>(*static_cast<C4_7x11::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 7 && h == 12) return runAnalysis<C4_7x12::GameSolver::Connect4::Solver, C4_7x12::GameSolver::Connect4::Position, C4_7x12::GameSolver::Connect4::Position::WIDTH, C4_7x12::GameSolver::Connect4::Position::HEIGHT, C4_7x12::GameSolver::Connect4::OpeningBookBase<7, 12>>(*static_cast<C4_7x12::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 4) return runAnalysis<C4_8x4::GameSolver::Connect4::Solver, C4_8x4::GameSolver::Connect4::Position, C4_8x4::GameSolver::Connect4::Position::WIDTH, C4_8x4::GameSolver::Connect4::Position::HEIGHT, C4_8x4::GameSolver::Connect4::OpeningBookBase<8, 4>>(*static_cast<C4_8x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 5) return runAnalysis<C4_8x5::GameSolver::Connect4::Solver, C4_8x5::GameSolver::Connect4::Position, C4_8x5::GameSolver::Connect4::Position::WIDTH, C4_8x5::GameSolver::Connect4::Position::HEIGHT, C4_8x5::GameSolver::Connect4::OpeningBookBase<8, 5>>(*static_cast<C4_8x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 7) return runAnalysis<C4_8x7::GameSolver::Connect4::Solver, C4_8x7::GameSolver::Connect4::Position, C4_8x7::GameSolver::Connect4::Position::WIDTH, C4_8x7::GameSolver::Connect4::Position::HEIGHT, C4_8x7::GameSolver::Connect4::OpeningBookBase<8, 7>>(*static_cast<C4_8x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 9) return runAnalysis<C4_8x9::GameSolver::Connect4::Solver, C4_8x9::GameSolver::Connect4::Position, C4_8x9::GameSolver::Connect4::Position::WIDTH, C4_8x9::GameSolver::Connect4::Position::HEIGHT, C4_8x9::GameSolver::Connect4::OpeningBookBase<8, 9>>(*static_cast<C4_8x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 10) return runAnalysis<C4_8x10::GameSolver::Connect4::Solver, C4_8x10::GameSolver::Connect4::Position, C4_8x10::GameSolver::Connect4::Position::WIDTH, C4_8x10::GameSolver::Connect4::Position::HEIGHT, C4_8x10::GameSolver::Connect4::OpeningBookBase<8, 10>>(*static_cast<C4_8x10::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 11) return runAnalysis<C4_8x11::GameSolver::Connect4::Solver, C4_8x11::GameSolver::Connect4::Position, C4_8x11::GameSolver::Connect4::Position::WIDTH, C4_8x11::GameSolver::Connect4::Position::HEIGHT, C4_8x11::GameSolver::Connect4::OpeningBookBase<8, 11>>(*static_cast<C4_8x11::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 12) return runAnalysis<C4_8x12::GameSolver::Connect4::Solver, C4_8x12::GameSolver::Connect4::Position, C4_8x12::GameSolver::Connect4::Position::WIDTH, C4_8x12::GameSolver::Connect4::Position::HEIGHT, C4_8x12::GameSolver::Connect4::OpeningBookBase<8, 12>>(*static_cast<C4_8x12::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 4) return runAnalysis<C4_9x4::GameSolver::Connect4::Solver, C4_9x4::GameSolver::Connect4::Position, C4_9x4::GameSolver::Connect4::Position::WIDTH, C4_9x4::GameSolver::Connect4::Position::HEIGHT, C4_9x4::GameSolver::Connect4::OpeningBookBase<9, 4>>(*static_cast<C4_9x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 5) return runAnalysis<C4_9x5::GameSolver::Connect4::Solver, C4_9x5::GameSolver::Connect4::Position, C4_9x5::GameSolver::Connect4::Position::WIDTH, C4_9x5::GameSolver::Connect4::Position::HEIGHT, C4_9x5::GameSolver::Connect4::OpeningBookBase<9, 5>>(*static_cast<C4_9x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 8) return runAnalysis<C4_9x8::GameSolver::Connect4::Solver, C4_9x8::GameSolver::Connect4::Position, C4_9x8::GameSolver::Connect4::Position::WIDTH, C4_9x8::GameSolver::Connect4::Position::HEIGHT, C4_9x8::GameSolver::Connect4::OpeningBookBase<9, 8>>(*static_cast<C4_9x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 10) return runAnalysis<C4_9x10::GameSolver::Connect4::Solver, C4_9x10::GameSolver::Connect4::Position, C4_9x10::GameSolver::Connect4::Position::WIDTH, C4_9x10::GameSolver::Connect4::Position::HEIGHT, C4_9x10::GameSolver::Connect4::OpeningBookBase<9, 10>>(*static_cast<C4_9x10::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 11) return runAnalysis<C4_9x11::GameSolver::Connect4::Solver, C4_9x11::GameSolver::Connect4::Position, C4_9x11::GameSolver::Connect4::Position::WIDTH, C4_9x11::GameSolver::Connect4::Position::HEIGHT, C4_9x11::GameSolver::Connect4::OpeningBookBase<9, 11>>(*static_cast<C4_9x11::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 12) return runAnalysis<C4_9x12::GameSolver::Connect4::Solver, C4_9x12::GameSolver::Connect4::Position, C4_9x12::GameSolver::Connect4::Position::WIDTH, C4_9x12::GameSolver::Connect4::Position::HEIGHT, C4_9x12::GameSolver::Connect4::OpeningBookBase<9, 12>>(*static_cast<C4_9x12::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 10 && h == 4) return runAnalysis<C4_10x4::GameSolver::Connect4::Solver, C4_10x4::GameSolver::Connect4::Position, C4_10x4::GameSolver::Connect4::Position::WIDTH, C4_10x4::GameSolver::Connect4::Position::HEIGHT, C4_10x4::GameSolver::Connect4::OpeningBookBase<10, 4>>(*static_cast<C4_10x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 10 && h == 5) return runAnalysis<C4_10x5::GameSolver::Connect4::Solver, C4_10x5::GameSolver::Connect4::Position, C4_10x5::GameSolver::Connect4::Position::WIDTH, C4_10x5::GameSolver::Connect4::Position::HEIGHT, C4_10x5::GameSolver::Connect4::OpeningBookBase<10, 5>>(*static_cast<C4_10x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 10 && h == 6) return runAnalysis<C4_10x6::GameSolver::Connect4::Solver, C4_10x6::GameSolver::Connect4::Position, C4_10x6::GameSolver::Connect4::Position::WIDTH, C4_10x6::GameSolver::Connect4::Position::HEIGHT, C4_10x6::GameSolver::Connect4::OpeningBookBase<10, 6>>(*static_cast<C4_10x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 10 && h == 8) return runAnalysis<C4_10x8::GameSolver::Connect4::Solver, C4_10x8::GameSolver::Connect4::Position, C4_10x8::GameSolver::Connect4::Position::WIDTH, C4_10x8::GameSolver::Connect4::Position::HEIGHT, C4_10x8::GameSolver::Connect4::OpeningBookBase<10, 8>>(*static_cast<C4_10x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 10 && h == 9) return runAnalysis<C4_10x9::GameSolver::Connect4::Solver, C4_10x9::GameSolver::Connect4::Position, C4_10x9::GameSolver::Connect4::Position::WIDTH, C4_10x9::GameSolver::Connect4::Position::HEIGHT, C4_10x9::GameSolver::Connect4::OpeningBookBase<10, 9>>(*static_cast<C4_10x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 10 && h == 11) return runAnalysis<C4_10x11::GameSolver::Connect4::Solver, C4_10x11::GameSolver::Connect4::Position, C4_10x11::GameSolver::Connect4::Position::WIDTH, C4_10x11::GameSolver::Connect4::Position::HEIGHT, C4_10x11::GameSolver::Connect4::OpeningBookBase<10, 11>>(*static_cast<C4_10x11::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 11 && h == 5) return runAnalysis<C4_11x5::GameSolver::Connect4::Solver, C4_11x5::GameSolver::Connect4::Position, C4_11x5::GameSolver::Connect4::Position::WIDTH, C4_11x5::GameSolver::Connect4::Position::HEIGHT, C4_11x5::GameSolver::Connect4::OpeningBookBase<11, 5>>(*static_cast<C4_11x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 11 && h == 6) return runAnalysis<C4_11x6::GameSolver::Connect4::Solver, C4_11x6::GameSolver::Connect4::Position, C4_11x6::GameSolver::Connect4::Position::WIDTH, C4_11x6::GameSolver::Connect4::Position::HEIGHT, C4_11x6::GameSolver::Connect4::OpeningBookBase<11, 6>>(*static_cast<C4_11x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 11 && h == 7) return runAnalysis<C4_11x7::GameSolver::Connect4::Solver, C4_11x7::GameSolver::Connect4::Position, C4_11x7::GameSolver::Connect4::Position::WIDTH, C4_11x7::GameSolver::Connect4::Position::HEIGHT, C4_11x7::GameSolver::Connect4::OpeningBookBase<11, 7>>(*static_cast<C4_11x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 11 && h == 8) return runAnalysis<C4_11x8::GameSolver::Connect4::Solver, C4_11x8::GameSolver::Connect4::Position, C4_11x8::GameSolver::Connect4::Position::WIDTH, C4_11x8::GameSolver::Connect4::Position::HEIGHT, C4_11x8::GameSolver::Connect4::OpeningBookBase<11, 8>>(*static_cast<C4_11x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 11 && h == 9) return runAnalysis<C4_11x9::GameSolver::Connect4::Solver, C4_11x9::GameSolver::Connect4::Position, C4_11x9::GameSolver::Connect4::Position::WIDTH, C4_11x9::GameSolver::Connect4::Position::HEIGHT, C4_11x9::GameSolver::Connect4::OpeningBookBase<11, 9>>(*static_cast<C4_11x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 11 && h == 10) return runAnalysis<C4_11x10::GameSolver::Connect4::Solver, C4_11x10::GameSolver::Connect4::Position, C4_11x10::GameSolver::Connect4::Position::WIDTH, C4_11x10::GameSolver::Connect4::Position::HEIGHT, C4_11x10::GameSolver::Connect4::OpeningBookBase<11, 10>>(*static_cast<C4_11x10::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 12 && h == 4) return runAnalysis<C4_12x4::GameSolver::Connect4::Solver, C4_12x4::GameSolver::Connect4::Position, C4_12x4::GameSolver::Connect4::Position::WIDTH, C4_12x4::GameSolver::Connect4::Position::HEIGHT, C4_12x4::GameSolver::Connect4::OpeningBookBase<12, 4>>(*static_cast<C4_12x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 12 && h == 5) return runAnalysis<C4_12x5::GameSolver::Connect4::Solver, C4_12x5::GameSolver::Connect4::Position, C4_12x5::GameSolver::Connect4::Position::WIDTH, C4_12x5::GameSolver::Connect4::Position::HEIGHT, C4_12x5::GameSolver::Connect4::OpeningBookBase<12, 5>>(*static_cast<C4_12x5::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 12 && h == 6) return runAnalysis<C4_12x6::GameSolver::Connect4::Solver, C4_12x6::GameSolver::Connect4::Position, C4_12x6::GameSolver::Connect4::Position::WIDTH, C4_12x6::GameSolver::Connect4::Position::HEIGHT, C4_12x6::GameSolver::Connect4::OpeningBookBase<12, 6>>(*static_cast<C4_12x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 12 && h == 7) return runAnalysis<C4_12x7::GameSolver::Connect4::Solver, C4_12x7::GameSolver::Connect4::Position, C4_12x7::GameSolver::Connect4::Position::WIDTH, C4_12x7::GameSolver::Connect4::Position::HEIGHT, C4_12x7::GameSolver::Connect4::OpeningBookBase<12, 7>>(*static_cast<C4_12x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 12 && h == 8) return runAnalysis<C4_12x8::GameSolver::Connect4::Solver, C4_12x8::GameSolver::Connect4::Position, C4_12x8::GameSolver::Connect4::Position::WIDTH, C4_12x8::GameSolver::Connect4::Position::HEIGHT, C4_12x8::GameSolver::Connect4::OpeningBookBase<12, 8>>(*static_cast<C4_12x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 12 && h == 9) return runAnalysis<C4_12x9::GameSolver::Connect4::Solver, C4_12x9::GameSolver::Connect4::Position, C4_12x9::GameSolver::Connect4::Position::WIDTH, C4_12x9::GameSolver::Connect4::Position::HEIGHT, C4_12x9::GameSolver::Connect4::OpeningBookBase<12, 9>>(*static_cast<C4_12x9::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 7) return runAnalysis<C4_9x7::GameSolver::Connect4::Solver, C4_9x7::GameSolver::Connect4::Position, 9, 7, C4_9x7::GameSolver::Connect4::OpeningBookBase<9, 7>>(*static_cast<C4_9x7::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 9 && h == 6) return runAnalysis<C4_9x6::GameSolver::Connect4::Solver, C4_9x6::GameSolver::Connect4::Position, 9, 6, C4_9x6::GameSolver::Connect4::OpeningBookBase<9, 6>>(*static_cast<C4_9x6::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 11 && h == 4) return runAnalysis<C4_11x4::GameSolver::Connect4::Solver, C4_11x4::GameSolver::Connect4::Position, 11, 4, C4_11x4::GameSolver::Connect4::OpeningBookBase<11, 4>>(*static_cast<C4_11x4::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    if (w == 8 && h == 8) return runAnalysis<C4_8x8::GameSolver::Connect4::Solver, C4_8x8::GameSolver::Connect4::Position, 8, 8, C4_8x8::GameSolver::Connect4::OpeningBookBase<8, 8>>(*static_cast<C4_8x8::GameSolver::Connect4::Solver*>(solver), pos, weak, threads, book_ptr, timeout_ms);
    return nullptr;
}

EMSCRIPTEN_KEEPALIVE int32_t* solveExact(int w, int h, void* solver, const char* pos, bool weak, void* book_ptr, double timeout_ms) {
    if (w == 6 && h == 5) return runSolve<C4_6x5::GameSolver::Connect4::Solver, C4_6x5::GameSolver::Connect4::Position, 6, 5, C4_6x5::GameSolver::Connect4::OpeningBookBase<6, 5>>(*static_cast<C4_6x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 6) return runSolve<C4_6x6::GameSolver::Connect4::Solver, C4_6x6::GameSolver::Connect4::Position, 6, 6, C4_6x6::GameSolver::Connect4::OpeningBookBase<6, 6>>(*static_cast<C4_6x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 6) return runSolve<C4_7x6::GameSolver::Connect4::Solver, C4_7x6::GameSolver::Connect4::Position, 7, 6, C4_7x6::GameSolver::Connect4::OpeningBookBase<7, 6>>(*static_cast<C4_7x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 7) return runSolve<C4_7x7::GameSolver::Connect4::Solver, C4_7x7::GameSolver::Connect4::Position, 7, 7, C4_7x7::GameSolver::Connect4::OpeningBookBase<7, 7>>(*static_cast<C4_7x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 6) return runSolve<C4_8x6::GameSolver::Connect4::Solver, C4_8x6::GameSolver::Connect4::Position, C4_8x6::GameSolver::Connect4::Position::WIDTH, C4_8x6::GameSolver::Connect4::Position::HEIGHT, C4_8x6::GameSolver::Connect4::OpeningBookBase<8, 6>>(*static_cast<C4_8x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 4) return runSolve<C4_4x4::GameSolver::Connect4::Solver, C4_4x4::GameSolver::Connect4::Position, C4_4x4::GameSolver::Connect4::Position::WIDTH, C4_4x4::GameSolver::Connect4::Position::HEIGHT, C4_4x4::GameSolver::Connect4::OpeningBookBase<4, 4>>(*static_cast<C4_4x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 5) return runSolve<C4_4x5::GameSolver::Connect4::Solver, C4_4x5::GameSolver::Connect4::Position, C4_4x5::GameSolver::Connect4::Position::WIDTH, C4_4x5::GameSolver::Connect4::Position::HEIGHT, C4_4x5::GameSolver::Connect4::OpeningBookBase<4, 5>>(*static_cast<C4_4x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 6) return runSolve<C4_4x6::GameSolver::Connect4::Solver, C4_4x6::GameSolver::Connect4::Position, C4_4x6::GameSolver::Connect4::Position::WIDTH, C4_4x6::GameSolver::Connect4::Position::HEIGHT, C4_4x6::GameSolver::Connect4::OpeningBookBase<4, 6>>(*static_cast<C4_4x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 7) return runSolve<C4_4x7::GameSolver::Connect4::Solver, C4_4x7::GameSolver::Connect4::Position, C4_4x7::GameSolver::Connect4::Position::WIDTH, C4_4x7::GameSolver::Connect4::Position::HEIGHT, C4_4x7::GameSolver::Connect4::OpeningBookBase<4, 7>>(*static_cast<C4_4x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 8) return runSolve<C4_4x8::GameSolver::Connect4::Solver, C4_4x8::GameSolver::Connect4::Position, C4_4x8::GameSolver::Connect4::Position::WIDTH, C4_4x8::GameSolver::Connect4::Position::HEIGHT, C4_4x8::GameSolver::Connect4::OpeningBookBase<4, 8>>(*static_cast<C4_4x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 9) return runSolve<C4_4x9::GameSolver::Connect4::Solver, C4_4x9::GameSolver::Connect4::Position, C4_4x9::GameSolver::Connect4::Position::WIDTH, C4_4x9::GameSolver::Connect4::Position::HEIGHT, C4_4x9::GameSolver::Connect4::OpeningBookBase<4, 9>>(*static_cast<C4_4x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 10) return runSolve<C4_4x10::GameSolver::Connect4::Solver, C4_4x10::GameSolver::Connect4::Position, C4_4x10::GameSolver::Connect4::Position::WIDTH, C4_4x10::GameSolver::Connect4::Position::HEIGHT, C4_4x10::GameSolver::Connect4::OpeningBookBase<4, 10>>(*static_cast<C4_4x10::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 11) return runSolve<C4_4x11::GameSolver::Connect4::Solver, C4_4x11::GameSolver::Connect4::Position, C4_4x11::GameSolver::Connect4::Position::WIDTH, C4_4x11::GameSolver::Connect4::Position::HEIGHT, C4_4x11::GameSolver::Connect4::OpeningBookBase<4, 11>>(*static_cast<C4_4x11::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 4 && h == 12) return runSolve<C4_4x12::GameSolver::Connect4::Solver, C4_4x12::GameSolver::Connect4::Position, C4_4x12::GameSolver::Connect4::Position::WIDTH, C4_4x12::GameSolver::Connect4::Position::HEIGHT, C4_4x12::GameSolver::Connect4::OpeningBookBase<4, 12>>(*static_cast<C4_4x12::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 4) return runSolve<C4_5x4::GameSolver::Connect4::Solver, C4_5x4::GameSolver::Connect4::Position, C4_5x4::GameSolver::Connect4::Position::WIDTH, C4_5x4::GameSolver::Connect4::Position::HEIGHT, C4_5x4::GameSolver::Connect4::OpeningBookBase<5, 4>>(*static_cast<C4_5x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 5) return runSolve<C4_5x5::GameSolver::Connect4::Solver, C4_5x5::GameSolver::Connect4::Position, C4_5x5::GameSolver::Connect4::Position::WIDTH, C4_5x5::GameSolver::Connect4::Position::HEIGHT, C4_5x5::GameSolver::Connect4::OpeningBookBase<5, 5>>(*static_cast<C4_5x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 6) return runSolve<C4_5x6::GameSolver::Connect4::Solver, C4_5x6::GameSolver::Connect4::Position, C4_5x6::GameSolver::Connect4::Position::WIDTH, C4_5x6::GameSolver::Connect4::Position::HEIGHT, C4_5x6::GameSolver::Connect4::OpeningBookBase<5, 6>>(*static_cast<C4_5x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 7) return runSolve<C4_5x7::GameSolver::Connect4::Solver, C4_5x7::GameSolver::Connect4::Position, C4_5x7::GameSolver::Connect4::Position::WIDTH, C4_5x7::GameSolver::Connect4::Position::HEIGHT, C4_5x7::GameSolver::Connect4::OpeningBookBase<5, 7>>(*static_cast<C4_5x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 8) return runSolve<C4_5x8::GameSolver::Connect4::Solver, C4_5x8::GameSolver::Connect4::Position, C4_5x8::GameSolver::Connect4::Position::WIDTH, C4_5x8::GameSolver::Connect4::Position::HEIGHT, C4_5x8::GameSolver::Connect4::OpeningBookBase<5, 8>>(*static_cast<C4_5x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 9) return runSolve<C4_5x9::GameSolver::Connect4::Solver, C4_5x9::GameSolver::Connect4::Position, C4_5x9::GameSolver::Connect4::Position::WIDTH, C4_5x9::GameSolver::Connect4::Position::HEIGHT, C4_5x9::GameSolver::Connect4::OpeningBookBase<5, 9>>(*static_cast<C4_5x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 10) return runSolve<C4_5x10::GameSolver::Connect4::Solver, C4_5x10::GameSolver::Connect4::Position, C4_5x10::GameSolver::Connect4::Position::WIDTH, C4_5x10::GameSolver::Connect4::Position::HEIGHT, C4_5x10::GameSolver::Connect4::OpeningBookBase<5, 10>>(*static_cast<C4_5x10::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 11) return runSolve<C4_5x11::GameSolver::Connect4::Solver, C4_5x11::GameSolver::Connect4::Position, C4_5x11::GameSolver::Connect4::Position::WIDTH, C4_5x11::GameSolver::Connect4::Position::HEIGHT, C4_5x11::GameSolver::Connect4::OpeningBookBase<5, 11>>(*static_cast<C4_5x11::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 5 && h == 12) return runSolve<C4_5x12::GameSolver::Connect4::Solver, C4_5x12::GameSolver::Connect4::Position, C4_5x12::GameSolver::Connect4::Position::WIDTH, C4_5x12::GameSolver::Connect4::Position::HEIGHT, C4_5x12::GameSolver::Connect4::OpeningBookBase<5, 12>>(*static_cast<C4_5x12::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 4) return runSolve<C4_6x4::GameSolver::Connect4::Solver, C4_6x4::GameSolver::Connect4::Position, C4_6x4::GameSolver::Connect4::Position::WIDTH, C4_6x4::GameSolver::Connect4::Position::HEIGHT, C4_6x4::GameSolver::Connect4::OpeningBookBase<6, 4>>(*static_cast<C4_6x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 7) return runSolve<C4_6x7::GameSolver::Connect4::Solver, C4_6x7::GameSolver::Connect4::Position, C4_6x7::GameSolver::Connect4::Position::WIDTH, C4_6x7::GameSolver::Connect4::Position::HEIGHT, C4_6x7::GameSolver::Connect4::OpeningBookBase<6, 7>>(*static_cast<C4_6x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 8) return runSolve<C4_6x8::GameSolver::Connect4::Solver, C4_6x8::GameSolver::Connect4::Position, C4_6x8::GameSolver::Connect4::Position::WIDTH, C4_6x8::GameSolver::Connect4::Position::HEIGHT, C4_6x8::GameSolver::Connect4::OpeningBookBase<6, 8>>(*static_cast<C4_6x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 9) return runSolve<C4_6x9::GameSolver::Connect4::Solver, C4_6x9::GameSolver::Connect4::Position, C4_6x9::GameSolver::Connect4::Position::WIDTH, C4_6x9::GameSolver::Connect4::Position::HEIGHT, C4_6x9::GameSolver::Connect4::OpeningBookBase<6, 9>>(*static_cast<C4_6x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 10) return runSolve<C4_6x10::GameSolver::Connect4::Solver, C4_6x10::GameSolver::Connect4::Position, C4_6x10::GameSolver::Connect4::Position::WIDTH, C4_6x10::GameSolver::Connect4::Position::HEIGHT, C4_6x10::GameSolver::Connect4::OpeningBookBase<6, 10>>(*static_cast<C4_6x10::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 11) return runSolve<C4_6x11::GameSolver::Connect4::Solver, C4_6x11::GameSolver::Connect4::Position, C4_6x11::GameSolver::Connect4::Position::WIDTH, C4_6x11::GameSolver::Connect4::Position::HEIGHT, C4_6x11::GameSolver::Connect4::OpeningBookBase<6, 11>>(*static_cast<C4_6x11::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 6 && h == 12) return runSolve<C4_6x12::GameSolver::Connect4::Solver, C4_6x12::GameSolver::Connect4::Position, C4_6x12::GameSolver::Connect4::Position::WIDTH, C4_6x12::GameSolver::Connect4::Position::HEIGHT, C4_6x12::GameSolver::Connect4::OpeningBookBase<6, 12>>(*static_cast<C4_6x12::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 4) return runSolve<C4_7x4::GameSolver::Connect4::Solver, C4_7x4::GameSolver::Connect4::Position, C4_7x4::GameSolver::Connect4::Position::WIDTH, C4_7x4::GameSolver::Connect4::Position::HEIGHT, C4_7x4::GameSolver::Connect4::OpeningBookBase<7, 4>>(*static_cast<C4_7x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 5) return runSolve<C4_7x5::GameSolver::Connect4::Solver, C4_7x5::GameSolver::Connect4::Position, C4_7x5::GameSolver::Connect4::Position::WIDTH, C4_7x5::GameSolver::Connect4::Position::HEIGHT, C4_7x5::GameSolver::Connect4::OpeningBookBase<7, 5>>(*static_cast<C4_7x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 8) return runSolve<C4_7x8::GameSolver::Connect4::Solver, C4_7x8::GameSolver::Connect4::Position, 7, 8, C4_7x8::GameSolver::Connect4::OpeningBookBase<7, 8>>(*static_cast<C4_7x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 9) return runSolve<C4_7x9::GameSolver::Connect4::Solver, C4_7x9::GameSolver::Connect4::Position, 7, 9, C4_7x9::GameSolver::Connect4::OpeningBookBase<7, 9>>(*static_cast<C4_7x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 10) return runSolve<C4_7x10::GameSolver::Connect4::Solver, C4_7x10::GameSolver::Connect4::Position, 7, 10, C4_7x10::GameSolver::Connect4::OpeningBookBase<7, 10>>(*static_cast<C4_7x10::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 11) return runSolve<C4_7x11::GameSolver::Connect4::Solver, C4_7x11::GameSolver::Connect4::Position, 7, 11, C4_7x11::GameSolver::Connect4::OpeningBookBase<7, 11>>(*static_cast<C4_7x11::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 7 && h == 12) return runSolve<C4_7x12::GameSolver::Connect4::Solver, C4_7x12::GameSolver::Connect4::Position, 7, 12, C4_7x12::GameSolver::Connect4::OpeningBookBase<7, 12>>(*static_cast<C4_7x12::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 4) return runSolve<C4_8x4::GameSolver::Connect4::Solver, C4_8x4::GameSolver::Connect4::Position, C4_8x4::GameSolver::Connect4::Position::WIDTH, C4_8x4::GameSolver::Connect4::Position::HEIGHT, C4_8x4::GameSolver::Connect4::OpeningBookBase<8, 4>>(*static_cast<C4_8x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 5) return runSolve<C4_8x5::GameSolver::Connect4::Solver, C4_8x5::GameSolver::Connect4::Position, C4_8x5::GameSolver::Connect4::Position::WIDTH, C4_8x5::GameSolver::Connect4::Position::HEIGHT, C4_8x5::GameSolver::Connect4::OpeningBookBase<8, 5>>(*static_cast<C4_8x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 7) return runSolve<C4_8x7::GameSolver::Connect4::Solver, C4_8x7::GameSolver::Connect4::Position, C4_8x7::GameSolver::Connect4::Position::WIDTH, C4_8x7::GameSolver::Connect4::Position::HEIGHT, C4_8x7::GameSolver::Connect4::OpeningBookBase<8, 7>>(*static_cast<C4_8x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 9) return runSolve<C4_8x9::GameSolver::Connect4::Solver, C4_8x9::GameSolver::Connect4::Position, C4_8x9::GameSolver::Connect4::Position::WIDTH, C4_8x9::GameSolver::Connect4::Position::HEIGHT, C4_8x9::GameSolver::Connect4::OpeningBookBase<8, 9>>(*static_cast<C4_8x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 10) return runSolve<C4_8x10::GameSolver::Connect4::Solver, C4_8x10::GameSolver::Connect4::Position, C4_8x10::GameSolver::Connect4::Position::WIDTH, C4_8x10::GameSolver::Connect4::Position::HEIGHT, C4_8x10::GameSolver::Connect4::OpeningBookBase<8, 10>>(*static_cast<C4_8x10::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 11) return runSolve<C4_8x11::GameSolver::Connect4::Solver, C4_8x11::GameSolver::Connect4::Position, C4_8x11::GameSolver::Connect4::Position::WIDTH, C4_8x11::GameSolver::Connect4::Position::HEIGHT, C4_8x11::GameSolver::Connect4::OpeningBookBase<8, 11>>(*static_cast<C4_8x11::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 12) return runSolve<C4_8x12::GameSolver::Connect4::Solver, C4_8x12::GameSolver::Connect4::Position, C4_8x12::GameSolver::Connect4::Position::WIDTH, C4_8x12::GameSolver::Connect4::Position::HEIGHT, C4_8x12::GameSolver::Connect4::OpeningBookBase<8, 12>>(*static_cast<C4_8x12::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 4) return runSolve<C4_9x4::GameSolver::Connect4::Solver, C4_9x4::GameSolver::Connect4::Position, C4_9x4::GameSolver::Connect4::Position::WIDTH, C4_9x4::GameSolver::Connect4::Position::HEIGHT, C4_9x4::GameSolver::Connect4::OpeningBookBase<9, 4>>(*static_cast<C4_9x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 5) return runSolve<C4_9x5::GameSolver::Connect4::Solver, C4_9x5::GameSolver::Connect4::Position, C4_9x5::GameSolver::Connect4::Position::WIDTH, C4_9x5::GameSolver::Connect4::Position::HEIGHT, C4_9x5::GameSolver::Connect4::OpeningBookBase<9, 5>>(*static_cast<C4_9x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 8) return runSolve<C4_9x8::GameSolver::Connect4::Solver, C4_9x8::GameSolver::Connect4::Position, C4_9x8::GameSolver::Connect4::Position::WIDTH, C4_9x8::GameSolver::Connect4::Position::HEIGHT, C4_9x8::GameSolver::Connect4::OpeningBookBase<9, 8>>(*static_cast<C4_9x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 10) return runSolve<C4_9x10::GameSolver::Connect4::Solver, C4_9x10::GameSolver::Connect4::Position, C4_9x10::GameSolver::Connect4::Position::WIDTH, C4_9x10::GameSolver::Connect4::Position::HEIGHT, C4_9x10::GameSolver::Connect4::OpeningBookBase<9, 10>>(*static_cast<C4_9x10::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 11) return runSolve<C4_9x11::GameSolver::Connect4::Solver, C4_9x11::GameSolver::Connect4::Position, C4_9x11::GameSolver::Connect4::Position::WIDTH, C4_9x11::GameSolver::Connect4::Position::HEIGHT, C4_9x11::GameSolver::Connect4::OpeningBookBase<9, 11>>(*static_cast<C4_9x11::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 12) return runSolve<C4_9x12::GameSolver::Connect4::Solver, C4_9x12::GameSolver::Connect4::Position, C4_9x12::GameSolver::Connect4::Position::WIDTH, C4_9x12::GameSolver::Connect4::Position::HEIGHT, C4_9x12::GameSolver::Connect4::OpeningBookBase<9, 12>>(*static_cast<C4_9x12::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 10 && h == 4) return runSolve<C4_10x4::GameSolver::Connect4::Solver, C4_10x4::GameSolver::Connect4::Position, C4_10x4::GameSolver::Connect4::Position::WIDTH, C4_10x4::GameSolver::Connect4::Position::HEIGHT, C4_10x4::GameSolver::Connect4::OpeningBookBase<10, 4>>(*static_cast<C4_10x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 10 && h == 5) return runSolve<C4_10x5::GameSolver::Connect4::Solver, C4_10x5::GameSolver::Connect4::Position, C4_10x5::GameSolver::Connect4::Position::WIDTH, C4_10x5::GameSolver::Connect4::Position::HEIGHT, C4_10x5::GameSolver::Connect4::OpeningBookBase<10, 5>>(*static_cast<C4_10x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 10 && h == 6) return runSolve<C4_10x6::GameSolver::Connect4::Solver, C4_10x6::GameSolver::Connect4::Position, C4_10x6::GameSolver::Connect4::Position::WIDTH, C4_10x6::GameSolver::Connect4::Position::HEIGHT, C4_10x6::GameSolver::Connect4::OpeningBookBase<10, 6>>(*static_cast<C4_10x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 10 && h == 8) return runSolve<C4_10x8::GameSolver::Connect4::Solver, C4_10x8::GameSolver::Connect4::Position, C4_10x8::GameSolver::Connect4::Position::WIDTH, C4_10x8::GameSolver::Connect4::Position::HEIGHT, C4_10x8::GameSolver::Connect4::OpeningBookBase<10, 8>>(*static_cast<C4_10x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 10 && h == 9) return runSolve<C4_10x9::GameSolver::Connect4::Solver, C4_10x9::GameSolver::Connect4::Position, C4_10x9::GameSolver::Connect4::Position::WIDTH, C4_10x9::GameSolver::Connect4::Position::HEIGHT, C4_10x9::GameSolver::Connect4::OpeningBookBase<10, 9>>(*static_cast<C4_10x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 10 && h == 11) return runSolve<C4_10x11::GameSolver::Connect4::Solver, C4_10x11::GameSolver::Connect4::Position, C4_10x11::GameSolver::Connect4::Position::WIDTH, C4_10x11::GameSolver::Connect4::Position::HEIGHT, C4_10x11::GameSolver::Connect4::OpeningBookBase<10, 11>>(*static_cast<C4_10x11::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 11 && h == 5) return runSolve<C4_11x5::GameSolver::Connect4::Solver, C4_11x5::GameSolver::Connect4::Position, C4_11x5::GameSolver::Connect4::Position::WIDTH, C4_11x5::GameSolver::Connect4::Position::HEIGHT, C4_11x5::GameSolver::Connect4::OpeningBookBase<11, 5>>(*static_cast<C4_11x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 11 && h == 6) return runSolve<C4_11x6::GameSolver::Connect4::Solver, C4_11x6::GameSolver::Connect4::Position, C4_11x6::GameSolver::Connect4::Position::WIDTH, C4_11x6::GameSolver::Connect4::Position::HEIGHT, C4_11x6::GameSolver::Connect4::OpeningBookBase<11, 6>>(*static_cast<C4_11x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 11 && h == 7) return runSolve<C4_11x7::GameSolver::Connect4::Solver, C4_11x7::GameSolver::Connect4::Position, C4_11x7::GameSolver::Connect4::Position::WIDTH, C4_11x7::GameSolver::Connect4::Position::HEIGHT, C4_11x7::GameSolver::Connect4::OpeningBookBase<11, 7>>(*static_cast<C4_11x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 11 && h == 8) return runSolve<C4_11x8::GameSolver::Connect4::Solver, C4_11x8::GameSolver::Connect4::Position, C4_11x8::GameSolver::Connect4::Position::WIDTH, C4_11x8::GameSolver::Connect4::Position::HEIGHT, C4_11x8::GameSolver::Connect4::OpeningBookBase<11, 8>>(*static_cast<C4_11x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 11 && h == 9) return runSolve<C4_11x9::GameSolver::Connect4::Solver, C4_11x9::GameSolver::Connect4::Position, C4_11x9::GameSolver::Connect4::Position::WIDTH, C4_11x9::GameSolver::Connect4::Position::HEIGHT, C4_11x9::GameSolver::Connect4::OpeningBookBase<11, 9>>(*static_cast<C4_11x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 11 && h == 10) return runSolve<C4_11x10::GameSolver::Connect4::Solver, C4_11x10::GameSolver::Connect4::Position, C4_11x10::GameSolver::Connect4::Position::WIDTH, C4_11x10::GameSolver::Connect4::Position::HEIGHT, C4_11x10::GameSolver::Connect4::OpeningBookBase<11, 10>>(*static_cast<C4_11x10::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 12 && h == 4) return runSolve<C4_12x4::GameSolver::Connect4::Solver, C4_12x4::GameSolver::Connect4::Position, C4_12x4::GameSolver::Connect4::Position::WIDTH, C4_12x4::GameSolver::Connect4::Position::HEIGHT, C4_12x4::GameSolver::Connect4::OpeningBookBase<12, 4>>(*static_cast<C4_12x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 12 && h == 5) return runSolve<C4_12x5::GameSolver::Connect4::Solver, C4_12x5::GameSolver::Connect4::Position, C4_12x5::GameSolver::Connect4::Position::WIDTH, C4_12x5::GameSolver::Connect4::Position::HEIGHT, C4_12x5::GameSolver::Connect4::OpeningBookBase<12, 5>>(*static_cast<C4_12x5::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 12 && h == 6) return runSolve<C4_12x6::GameSolver::Connect4::Solver, C4_12x6::GameSolver::Connect4::Position, 12, 6, C4_12x6::GameSolver::Connect4::OpeningBookBase<12, 6>>(*static_cast<C4_12x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 12 && h == 7) return runSolve<C4_12x7::GameSolver::Connect4::Solver, C4_12x7::GameSolver::Connect4::Position, 12, 7, C4_12x7::GameSolver::Connect4::OpeningBookBase<12, 7>>(*static_cast<C4_12x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 12 && h == 8) return runSolve<C4_12x8::GameSolver::Connect4::Solver, C4_12x8::GameSolver::Connect4::Position, 12, 8, C4_12x8::GameSolver::Connect4::OpeningBookBase<12, 8>>(*static_cast<C4_12x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 12 && h == 9) return runSolve<C4_12x9::GameSolver::Connect4::Solver, C4_12x9::GameSolver::Connect4::Position, 12, 9, C4_12x9::GameSolver::Connect4::OpeningBookBase<12, 9>>(*static_cast<C4_12x9::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 7) return runSolve<C4_9x7::GameSolver::Connect4::Solver, C4_9x7::GameSolver::Connect4::Position, 9, 7, C4_9x7::GameSolver::Connect4::OpeningBookBase<9, 7>>(*static_cast<C4_9x7::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 9 && h == 6) return runSolve<C4_9x6::GameSolver::Connect4::Solver, C4_9x6::GameSolver::Connect4::Position, 9, 6, C4_9x6::GameSolver::Connect4::OpeningBookBase<9, 6>>(*static_cast<C4_9x6::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 11 && h == 4) return runSolve<C4_11x4::GameSolver::Connect4::Solver, C4_11x4::GameSolver::Connect4::Position, 11, 4, C4_11x4::GameSolver::Connect4::OpeningBookBase<11, 4>>(*static_cast<C4_11x4::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    if (w == 8 && h == 8) return runSolve<C4_8x8::GameSolver::Connect4::Solver, C4_8x8::GameSolver::Connect4::Position, 8, 8, C4_8x8::GameSolver::Connect4::OpeningBookBase<8, 8>>(*static_cast<C4_8x8::GameSolver::Connect4::Solver*>(solver), pos, weak, book_ptr, timeout_ms);
    return nullptr;
}
...
EMSCRIPTEN_KEEPALIVE int32_t* analyzeHeuristic(int w, int h, void* solver, const char* pos, int threads, int max_depth, double timeout_ms) {
    ...
    if (w == 6 && h == 8) return runHeuristicAnalysis<GameSolver::Connect4::HeuristicSolver<6, 8>, GameSolver::Connect4::GenericPosition<6, 8>, 8>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 8>*>(solver), pos, max_depth, threads, timeout_ms);
    ...
    if (w == 7 && h == 8) return runHeuristicAnalysis<GameSolver::Connect4::HeuristicSolver<7, 8>, GameSolver::Connect4::GenericPosition<7, 8>, 8>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 8>*>(solver), pos, max_depth, threads, timeout_ms);
    ...
    if (w == 9 && h == 8) return runHeuristicAnalysis<GameSolver::Connect4::HeuristicSolver<9, 8>, GameSolver::Connect4::GenericPosition<9, 8>, 8>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 8>*>(solver), pos, max_depth, threads, timeout_ms);
    ...
    if (w == 10 && h == 8) return runHeuristicAnalysis<GameSolver::Connect4::HeuristicSolver<10, 8>, GameSolver::Connect4::GenericPosition<10, 8>, 8>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 8>*>(solver), pos, max_depth, threads, timeout_ms);
...
EMSCRIPTEN_KEEPALIVE uint64_t getNodeCount(int w, int h, void* solver, bool is_heuristic) {
    if (is_heuristic) {
        ...
        if (w == 6 && h == 8) return static_cast<GameSolver::Connect4::HeuristicSolver<6, 8>*>(solver)->getNodeCount();
        ...
        if (w == 7 && h == 8) return static_cast<GameSolver::Connect4::HeuristicSolver<7, 8>*>(solver)->getNodeCount();
        ...
        if (w == 9 && h == 8) return static_cast<GameSolver::Connect4::HeuristicSolver<9, 8>*>(solver)->getNodeCount();
        ...
        if (w == 10 && h == 8) return static_cast<GameSolver::Connect4::HeuristicSolver<10, 8>*>(solver)->getNodeCount();
        ...
        if (w == 11 && h == 8) return static_cast<GameSolver::Connect4::HeuristicSolver<11, 8>*>(solver)->getNodeCount();
        ...
        if (w == 12 && h == 8) return static_cast<GameSolver::Connect4::HeuristicSolver<12, 8>*>(solver)->getNodeCount();
...
int32_t* runSolveHeuristic(CoreSolver& solver, const char* positionCharArr, int max_depth, void* book_ptr, double timeout_ms) {
  std::string positionString(positionCharArr);
  CorePosition P;
...
EMSCRIPTEN_KEEPALIVE int32_t* solveHeuristic(int w, int h, void* solver, const char* pos, int max_depth, double timeout_ms, void* book_ptr) {
    if (w == 6 && h == 5) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 5>, GameSolver::Connect4::GenericPosition<6, 5>, 6, 5, GameSolver::Connect4::OpeningBookBase<6, 5>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 5>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 6 && h == 6) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 6>, GameSolver::Connect4::GenericPosition<6, 6>, 6, 6, GameSolver::Connect4::OpeningBookBase<6, 6>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 6>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 6) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 6>, GameSolver::Connect4::GenericPosition<7, 6>, 7, 6, GameSolver::Connect4::OpeningBookBase<7, 6>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 6>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 7) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 7>, GameSolver::Connect4::GenericPosition<7, 7>, 7, 7, GameSolver::Connect4::OpeningBookBase<7, 7>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 7>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 6) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 6>, GameSolver::Connect4::GenericPosition<8, 6>, 8, 6, GameSolver::Connect4::OpeningBookBase<8, 6>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 6>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 6 && h == 7) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 7>, GameSolver::Connect4::GenericPosition<6, 7>, 6, 7, GameSolver::Connect4::OpeningBookBase<6, 7>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 7>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 6 && h == 8) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 8>, GameSolver::Connect4::GenericPosition<6, 8>, 6, 8, GameSolver::Connect4::OpeningBookBase<6, 8>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 8>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 6 && h == 9) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 9>, GameSolver::Connect4::GenericPosition<6, 9>, 6, 9, GameSolver::Connect4::OpeningBookBase<6, 9>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 9>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 6 && h == 10) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 10>, GameSolver::Connect4::GenericPosition<6, 10>, 6, 10, GameSolver::Connect4::OpeningBookBase<6, 10>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 10>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 6 && h == 11) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 11>, GameSolver::Connect4::GenericPosition<6, 11>, 6, 11, GameSolver::Connect4::OpeningBookBase<6, 11>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 11>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 6 && h == 12) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<6, 12>, GameSolver::Connect4::GenericPosition<6, 12>, 6, 12, GameSolver::Connect4::OpeningBookBase<6, 12>>(*static_cast<GameSolver::Connect4::HeuristicSolver<6, 12>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 5) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 5>, GameSolver::Connect4::GenericPosition<7, 5>, 7, 5, GameSolver::Connect4::OpeningBookBase<7, 5>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 5>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 8) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 8>, GameSolver::Connect4::GenericPosition<7, 8>, 7, 8, GameSolver::Connect4::OpeningBookBase<7, 8>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 8>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 9) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 9>, GameSolver::Connect4::GenericPosition<7, 9>, 7, 9, GameSolver::Connect4::OpeningBookBase<7, 9>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 9>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 10) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 10>, GameSolver::Connect4::GenericPosition<7, 10>, 7, 10, GameSolver::Connect4::OpeningBookBase<7, 10>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 10>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 11) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 11>, GameSolver::Connect4::GenericPosition<7, 11>, 7, 11, GameSolver::Connect4::OpeningBookBase<7, 11>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 11>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 7 && h == 12) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<7, 12>, GameSolver::Connect4::GenericPosition<7, 12>, 7, 12, GameSolver::Connect4::OpeningBookBase<7, 12>>(*static_cast<GameSolver::Connect4::HeuristicSolver<7, 12>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 5) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 5>, GameSolver::Connect4::GenericPosition<8, 5>, 8, 5, GameSolver::Connect4::OpeningBookBase<8, 5>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 5>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 7) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 7>, GameSolver::Connect4::GenericPosition<8, 7>, 8, 7, GameSolver::Connect4::OpeningBookBase<8, 7>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 7>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 9) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 9>, GameSolver::Connect4::GenericPosition<8, 9>, 8, 9, GameSolver::Connect4::OpeningBookBase<8, 9>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 9>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 10) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 10>, GameSolver::Connect4::GenericPosition<8, 10>, 8, 10, GameSolver::Connect4::OpeningBookBase<8, 10>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 10>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 11) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 11>, GameSolver::Connect4::GenericPosition<8, 11>, 8, 11, GameSolver::Connect4::OpeningBookBase<8, 11>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 11>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 12) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 12>, GameSolver::Connect4::GenericPosition<8, 12>, 8, 12, GameSolver::Connect4::OpeningBookBase<8, 12>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 12>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 5) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 5>, GameSolver::Connect4::GenericPosition<9, 5>, 9, 5, GameSolver::Connect4::OpeningBookBase<9, 5>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 5>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 8) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 8>, GameSolver::Connect4::GenericPosition<9, 8>, 9, 8, GameSolver::Connect4::OpeningBookBase<9, 8>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 8>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 10) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 10>, GameSolver::Connect4::GenericPosition<9, 10>, 9, 10, GameSolver::Connect4::OpeningBookBase<9, 10>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 10>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 11) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 11>, GameSolver::Connect4::GenericPosition<9, 11>, 9, 11, GameSolver::Connect4::OpeningBookBase<9, 11>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 11>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 12) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 12>, GameSolver::Connect4::GenericPosition<9, 12>, 9, 12, GameSolver::Connect4::OpeningBookBase<9, 12>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 12>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 10 && h == 5) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<10, 5>, GameSolver::Connect4::GenericPosition<10, 5>, 10, 5, GameSolver::Connect4::OpeningBookBase<10, 5>>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 5>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 10 && h == 6) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<10, 6>, GameSolver::Connect4::GenericPosition<10, 6>, 10, 6, GameSolver::Connect4::OpeningBookBase<10, 6>>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 6>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 10 && h == 8) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<10, 8>, GameSolver::Connect4::GenericPosition<10, 8>, 10, 8, GameSolver::Connect4::OpeningBookBase<10, 8>>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 8>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 10 && h == 9) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<10, 9>, GameSolver::Connect4::GenericPosition<10, 9>, 10, 9, GameSolver::Connect4::OpeningBookBase<10, 9>>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 9>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 10 && h == 11) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<10, 11>, GameSolver::Connect4::GenericPosition<10, 11>, 10, 11, GameSolver::Connect4::OpeningBookBase<10, 11>>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 11>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 11 && h == 5) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<11, 5>, GameSolver::Connect4::GenericPosition<11, 5>, 11, 5, GameSolver::Connect4::OpeningBookBase<11, 5>>(*static_cast<GameSolver::Connect4::HeuristicSolver<11, 5>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 11 && h == 6) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<11, 6>, GameSolver::Connect4::GenericPosition<11, 6>, 11, 6, GameSolver::Connect4::OpeningBookBase<11, 6>>(*static_cast<GameSolver::Connect4::HeuristicSolver<11, 6>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 11 && h == 7) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<11, 7>, GameSolver::Connect4::GenericPosition<11, 7>, 11, 7, GameSolver::Connect4::OpeningBookBase<11, 7>>(*static_cast<GameSolver::Connect4::HeuristicSolver<11, 7>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 11 && h == 8) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<11, 8>, GameSolver::Connect4::GenericPosition<11, 8>, 11, 8, GameSolver::Connect4::OpeningBookBase<11, 8>>(*static_cast<GameSolver::Connect4::HeuristicSolver<11, 8>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 11 && h == 9) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<11, 9>, GameSolver::Connect4::GenericPosition<11, 9>, 11, 9, GameSolver::Connect4::OpeningBookBase<11, 9>>(*static_cast<GameSolver::Connect4::HeuristicSolver<11, 9>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 11 && h == 10) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<11, 10>, GameSolver::Connect4::GenericPosition<11, 10>, 11, 10, GameSolver::Connect4::OpeningBookBase<11, 10>>(*static_cast<GameSolver::Connect4::HeuristicSolver<11, 10>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 12 && h == 5) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<12, 5>, GameSolver::Connect4::GenericPosition<12, 5>, 12, 5, GameSolver::Connect4::OpeningBookBase<12, 5>>(*static_cast<GameSolver::Connect4::HeuristicSolver<12, 5>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 12 && h == 6) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<12, 6>, GameSolver::Connect4::GenericPosition<12, 6>, 12, 6, GameSolver::Connect4::OpeningBookBase<12, 6>>(*static_cast<GameSolver::Connect4::HeuristicSolver<12, 6>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 12 && h == 7) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<12, 7>, GameSolver::Connect4::GenericPosition<12, 7>, 12, 7, GameSolver::Connect4::OpeningBookBase<12, 7>>(*static_cast<GameSolver::Connect4::HeuristicSolver<12, 7>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 12 && h == 8) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<12, 8>, GameSolver::Connect4::GenericPosition<12, 8>, 12, 8, GameSolver::Connect4::OpeningBookBase<12, 8>>(*static_cast<GameSolver::Connect4::HeuristicSolver<12, 8>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 12 && h == 9) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<12, 9>, GameSolver::Connect4::GenericPosition<12, 9>, 12, 9, GameSolver::Connect4::OpeningBookBase<12, 9>>(*static_cast<GameSolver::Connect4::HeuristicSolver<12, 9>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 7) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 7>, GameSolver::Connect4::GenericPosition<9, 7>, 9, 7, GameSolver::Connect4::OpeningBookBase<9, 7>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 7>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 8 && h == 8) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<8, 8>, GameSolver::Connect4::GenericPosition<8, 8>, 8, 8, GameSolver::Connect4::OpeningBookBase<8, 8>>(*static_cast<GameSolver::Connect4::HeuristicSolver<8, 8>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 10 && h == 7) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<10, 7>, GameSolver::Connect4::GenericPosition<10, 7>, 10, 7, GameSolver::Connect4::OpeningBookBase<10, 7>>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 7>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 9) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 9>, GameSolver::Connect4::GenericPosition<9, 9>, 9, 9, GameSolver::Connect4::OpeningBookBase<9, 9>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 9>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 10 && h == 10) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<10, 10>, GameSolver::Connect4::GenericPosition<10, 10>, 10, 10, GameSolver::Connect4::OpeningBookBase<10, 10>>(*static_cast<GameSolver::Connect4::HeuristicSolver<10, 10>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 9 && h == 6) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<9, 6>, GameSolver::Connect4::GenericPosition<9, 6>, 9, 6, GameSolver::Connect4::OpeningBookBase<9, 6>>(*static_cast<GameSolver::Connect4::HeuristicSolver<9, 6>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    if (w == 11 && h == 4) return runSolveHeuristic<GameSolver::Connect4::HeuristicSolver<11, 4>, GameSolver::Connect4::GenericPosition<11, 4>, 11, 4, GameSolver::Connect4::OpeningBookBase<11, 4>>(*static_cast<GameSolver::Connect4::HeuristicSolver<11, 4>*>(solver), pos, max_depth, book_ptr, timeout_ms);
    return nullptr;
}
