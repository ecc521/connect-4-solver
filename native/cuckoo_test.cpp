...
int main(int argc, char** argv) {
  ...
  // Cuckoo table targets a highly compact 90% load factor implicitly.
  // 4 items per bucket. So num_buckets = (num_lines / 0.90) / 4
  ...
  while (true) {
    ...
    std::cerr << "Graph full (" << fails << " evictions). Expanding by 5%...\n";
    delete table;
    target_buckets = next_prime(target_buckets * 1.05);
  }
