...
export abstract class BaseConnect4Solver {
  ...
  protected sanitizeOpts(opts?: { threads?: number, maxDepth?: number, timeoutMs?: number, book?: any }) {
    let threads = opts?.threads ?? 1;
    if (typeof threads !== "number" || isNaN(threads) || threads < 1 || !isFinite(threads)) throw new Error("Invalid 'threads' parameter.");
    threads = Math.floor(threads);

    let requestedMaxDepth = opts?.maxDepth ?? (this.width * this.height);
    if (typeof requestedMaxDepth !== "number" || isNaN(requestedMaxDepth) || requestedMaxDepth < 1 || !isFinite(requestedMaxDepth)) throw new Error("Invalid 'maxDepth' parameter.");
    const maxDepth = Math.min(Math.floor(requestedMaxDepth), this.width * this.height);

    let timeoutMs = opts?.timeoutMs ?? 25;
