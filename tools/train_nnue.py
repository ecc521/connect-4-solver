...
Accepts two types of .bin files:
  --exact  data_exact.bin  : 40 bytes/record:
  ...
  --soft   data_soft.bin   : 40 bytes/record: same layout, float32 score instead of int32.
...
class ExactDataset(Dataset):
    ...
    def __init__(self, files, width, height):
        ...
        # ── Sanity check 1: board overlap ────────────────────────────────────
        # In any legal Connect-4 position, no square can be occupied by both
        # players simultaneously. If this fires, the file is corrupt or was
        # generated in an incompatible format (e.g. old 20-byte records being
        # read by the 40-byte parser, or a static_cast<uint64_t> truncation).
        overlap = ((data['pos_lo'] & data['opp_lo']) | (data['pos_hi'] & data['opp_hi'])) != np.uint64(0)
        overlap_rate = overlap.mean()
        if overlap_rate > 0.001:
            raise ValueError(
                f"Dataset integrity check FAILED: {overlap_rate*100:.2f}% of records have "
                f"overlapping pieces (pos & opp != 0). This almost certainly means the .bin "
                f"file was written in a different format than the 40-byte lo/hi layout "
                f"ExactDataset expects. Check that the file was generated by "
                f"gen_training_data_8x8.cpp (post-commit 334e753), not by the legacy "
                f"gen_dataset_parallel.py or any tool that used static_cast<uint64_t>."
            )

        # ── Sanity check 2: score outlier rate ───────────────────────────────
        # Genuine datasets should have virtually all scores within the minimax
        # range. A clip rate > 1% means the score field is being read from the
        # wrong byte offset (format mismatch) or the data is otherwise corrupt.
        mask = (data['score'] >= -self.max_score) & (data['score'] <= self.max_score)
        outlier_rate = (~mask).mean()
        if outlier_rate > 0.01:
            raise ValueError(
                f"Dataset integrity check FAILED: {outlier_rate*100:.2f}% of scores are "
                f"outside [-{self.max_score}, +{self.max_score}]. Expected < 1%. "
                f"The score field is likely being read from the wrong byte offset — "
                f"check that record size and field order match the 40-byte ExactRecord layout."
            )
        data = data[mask]
        print(f"Exact: {len(data)} records (filtered {(~mask).sum()} outliers, overlap_rate={overlap_rate*100:.3f}%)")
...
class SoftDataset(Dataset):
    ...
    def __init__(self, files, width, height, loss_weight=0.3):
        ...
        if overlap_rate > 0.001:
            raise ValueError(
                f"Dataset integrity check FAILED: {overlap_rate*100:.2f}% of soft records have "
                f"overlapping pieces (pos & opp != 0). File is likely corrupt or in a mismatched format."
            )
        ...
        if outlier_rate > 0.01:
            raise ValueError(
                f"Dataset integrity check FAILED: {outlier_rate*100:.2f}% of soft scores are "
                f"outside [-1.0, +1.0]. Expected < 1%. File is likely corrupt or misformatted."
            )
        data = data[mask]
        print(f"Soft: {len(data)} records (filtered {(~mask).sum()} outliers, overlap_rate={overlap_rate*100:.3f}%)")
